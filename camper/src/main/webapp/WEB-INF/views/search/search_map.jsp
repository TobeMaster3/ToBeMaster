...
<script type="text/javascript">
	...
	// 고캠핑 API에서 캠핑장 리스트를 얻어오는 함수
	const getMapList = (pageNo)=>{
		// 이용자가 검색한 키워드를 변수로 저장
		let strSearchMap= document.getElementById("strSearchMap").value;
		// 검색 결과를 노출시킬 컨테이너를 변수로 저장
		let container = document.getElementById('map');
		// 검색 전에는 컨테이너를 노출시키지 않음
					...
					request.onreadystatechange = function() {
						if(request.readyState==4){
							if(request.status==200){
								...
								// 고캠핑 API에서 얻어온 좌표정보로 카카오맵 API에 적용해서 마커를 찍고 보여줌
								//
								let options = {
							    ...
							    // 마커 이미지를 생성합니다    
									...
									// 카카오맵 API를 이용해 지도 위에 marker를 찍음
			...
			<div class="row">
				<div class="col-md-12">
					<div class="section-title">
						<div id="map" class="container col-md-12" style="width:600px;height:400px; display:none">
						</div>
					</div>
				</div>
			</div>
